...
def getHand(colorframe, uncaliColorframe, colorspace, edges):
    mode = cv2.RETR_EXTERNAL  # cv2.RETR_LIST
    method = cv2.CHAIN_APPROX_SIMPLE
    handMask = gethandmask(colorframe, colorspace, uncaliColorframe)  # hand mask
    hands = getcontourmask(handMask, edges) # divide the big mask into individual hand masks and contours
    for hand in hands:
